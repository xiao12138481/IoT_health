 ...
 * - 低于最小值返回'low'（偏低）
 * - 在正常范围内返回'normal'（正常）
 * @param {number} value - 体温值
 * @param {number} min - 体温正常范围最小值
 * @param {number} max - 体温正常范围最大值
 * @returns {'high' | 'low' | 'normal'} 体温状态
 */
...
export default function UserBodyTemperaturePage() {
      ...
      <div className="p-6 space-y-6">
          ...
          <div className="flex gap-2">
            {(['1d', '3d', '7d'] as const).map((r) => (
              <Button
                ...
                size="sm"
                onClick={() => setRange(r)}
                className={range === r ? 'bg-orange-500 hover:bg-orange-600' : ''}
              >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            { label: '最高体温', value: max !== null && Number.isFinite(max) ? max.toFixed(1) : '--', unit: '°C', icon: TrendingUp, color: 'text-red-600' },
