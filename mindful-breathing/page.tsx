...
export default function MindfulBreathingPage() {
  const { persons, currentPersonId, setCurrentPersonId, alarmCount, isLoading } = useApp();
  /*训练场景和会话状态*/
  ...
  useEffect(() => {
    ...
    async function loadBaseline() {
      ...
      try {
        ...
        const fallbackName = persons.find((person) => person.id === currentPersonId)?.name ?? '';
        setPersonName(data.person?.name ?? fallbackName);
        setBaseline({
            ...
            <CardHeader className="pb-3">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  ...
                  <CardDescription>
                    当前对象：{personName || '监测对象'}，建议在安静坐姿下完成本轮训练。
                  </CardDescription>
                </div>
                <div className="flex flex-wrap gap-2">
